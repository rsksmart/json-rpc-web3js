...
describe('Rskj rsk3js Smoke Tests', function () {
  ...
  it('Should have all the simple cached methods work', async () => {
    ...
    // eth_hashrate
    let hashRate = await rsk3.eth.getHashrate();
    assert.equal(hashRate, '0');

    // eth_syncing
    let isSyncing = await rsk3.eth.isSyncing();
    ...
    // net_listening
    let isListening = await rsk3.eth.net.isListening();
    assert(isListening);

    // net_peerCount
    let peerCount = await rsk3.eth.net.getPeerCount();
    assert.isAbove(peerCount, -1);


    // net_version
    let networkId = await rsk3.eth.net.getId();
    assert.equal(networkId, 33);

    // eth_accounts
    let accounts = await rsk3.eth.getAccounts();
    assert.isArray(accounts);

    // eth_protocolVersion
    let protocolVersion = await rsk3.eth.getProtocolVersion();
    assert.equal(protocolVersion, '62');

    // eth_mining
    let isMining = await rsk3.eth.isMining();
  ...
  it('eth_getBalance: Should get a the right balance for an account', async () => {
    let balance = new BN(await rsk3.eth.getBalance(
      testAccount,
      'latest'
    ));
    assert.equal(balance.toNumber(), '21000000000000000000000000');

    let historicBalance = new BN(await rsk3.eth.getBalance(
      testAccount,
      0
    ));
    ...
    let unusedAccountBalance = new BN(await rsk3.eth.getBalance(
  ...
  it('eth_getTransactionByBlockHashAndIndex-eth_getTransactionByBlockNumberAndIndex: Should get transactions by block number and hash', async () => {
    let blockNumber = 1;
    let byBlockNumber = await rsk3.eth.getTransactionFromBlock(blockNumber, 0);
    let blockHash = byBlockNumber.blockHash;
    let byHash = await rsk3.eth.getTransactionFromBlock(blockHash, 0);

    assert.deepEqual(byBlockNumber, byHash);

    let invalidBlock = await rsk3.eth.getTransactionFromBlock('0xdeadbeef0fb9424aad2417321cac62915f6c83827f4d3c8c8c06900a61c4236c', 0);
  ...
  it(`eth_getBlockTransactionCountByHash-eth_getBlockTransactionCountByNumber: Should return the right number of block transactions`, async () => {
    ...
    let byHash = await rsk3.eth.getBlockTransactionCount(blockHash);
    assert.equal(byHash, expectedCount);

    let byNumber = await rsk3.eth.getBlockTransactionCount(blockNumber);
  ...
  it('eth_sendRawTransaction & eth_call: Should compile and deploy a contract successfully and interact with that contract', async function () {
    ...
    let contract = new rsk3.eth.Contract(abi);
    let signedAccount = rsk3.eth.accounts.privateKeyToAccount(PRIVATE_KEY);
    ...
    let txReceipt = await rsk3.eth.sendSignedTransaction(signedTx.rawTransaction);
    assert(txReceipt.contractAddress);
    contractAddress = txReceipt.contractAddress;

    let deployedContract = new rsk3.eth.Contract(abi, txReceipt.contractAddress);
    ...
    let receipt = await rsk3.eth.sendSignedTransaction(setSignedTx.rawTransaction)
  ...
  it(`eth_getCode: Should return the contract's code`, async () => {
    let contractCode = await rsk3.eth.getCode(contractAddress, 'latest');
    ...
    let accountCount = await rsk3.eth.getCode(testAccount, 'earliest');
    assert.equal('0x00', accountCount);

    let invalidAccount = await rsk3.eth.getCode('0x0000000000000000000000000000000000000001', 'latest');
  ...
  it(`eth_getBlockByHash-eth_getBlockByNumber: Should get the block by hash and number`, async () => {
    this.timeout(20000);
    let blockNumber = '2';
    let byNumber = await rsk3.eth.getBlock(blockNumber);
    let blockHash = byNumber.hash;
    let byHash = await rsk3.eth.getBlock(blockHash);

    assert.deepEqual(byHash, byNumber);

    let withTransactions = await rsk3.eth.getBlock(blockHash, true);
  ...
  it(`eth_getTransactionByHash: Should get a transaction by its hash`, async () => {
    let tx = await rsk3.eth.getTransaction(trxHash);
    assert.isObject(tx);
    ...
    let invalidTx = await rsk3.eth.getTransaction('0x5eae996aa609c0b9db434c7a2411437fefc3ff16046b71ad102453cfdeadbeef');
  ...
  it(`eth_getTransactionReceipt: Should get transaction receipt`, async () => {
    ...
    let receipt = await rsk3.eth.getTransactionReceipt(trxHash);
    assert.isObject(receipt);
    let receiptString = JSON.stringify(receipt);
    assert(receiptString.indexOf('transactionHash') >= 0, "transactionHash is not being returned and it's expected!");
    assert(receiptString.indexOf('transactionIndex') >= 0, "transactionIndex is not being returned and it's expected!");
    ...
    assert(receiptString.indexOf('status') >= 0, "status is not being returned and it's expected!");
    assert(receiptString.indexOf('logsBloom') >= 0, "logsBloom is not being returned and it's expected!");
    let invalidTx = await rsk3.eth.getTransactionReceipt('0xd05274b72ca6346bcce89a64cd42ddd28d885fdd06772efe0fe7d19fdeadbeef');
    assert.isNull(invalidTx);
  });

  // eth_getStorageAt
  it(`eth_getStorageAt: Should get storage at a specific location`, async () => {
    let storageValue = await rsk3.eth.getStorageAt(
      contractAddress,
      0,
      'latest'
    );
  ...
  it(`eth_getLogs: Should get the logs of a contract`, async () => {
    let logs = await rsk3.eth.getPastLogs({
      'fromBlock': "0x0",
      'toBlock': await rsk3.eth.getBlockNumber(),
  ...
  it(`eth_coinbase: Should return coinbase from RskJ`, async () => {
    let coinbase = await rsk3.eth.getCoinbase();
